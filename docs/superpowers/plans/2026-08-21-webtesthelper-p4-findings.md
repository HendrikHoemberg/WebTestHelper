...
Plan 4 executed with subagent-driven development: six task commits plus eight review-fix commits,
350 `-Pfast` / 417 full tests green, three-run browser acceptance included.

- **The plan's verbatim `Fingerprint` join fails its own splice test.** Joining with `\0` and no
  escaping makes `("a\0b","c")` and `("a","b\0c")` identical bytes; the NUL-doubling first fix was
  still collidable (`("x\0","y")` vs `("x","\0y")`). Final design: separator is **U+0001**, no
  escaping — injective over the domain because no valid key (URL, `CheckType` name, decimal site
  id) contains a control byte. The javadoc states the domain contract, and `FingerprintTest` pins
  bytes-in-values; do not "fix" it by making keys separator-safe.
- **`RunCoverage.of` uses strict `normalize` + drop, not `locationKeyOf`.** The plan's two
  requirements conflict: `locationKeyOf` falls back to the raw value for unparseable input, so
  "unparseable does not appear" would fail with it. Scheme-less URLs are dropped by `normalize`,
  so the plan's `/kontakt.html` test examples needed absolute URLs.
- **`Severity` is declared `ERROR, WARN, INFO` (most-severe-first) and `Severity.max` relies on
  that.** It was pre-existing dead code until the materializer adopted it for the representative
  occurrence; its javadoc now warns against reordering the constants. The representative message
  is the highest-severity occurrence's, ties broken by lowest `pageUrl` (`nullsFirst`).
- **Materialiser details pinned by tests:** occurrence dedupe key = canonical URL
  (`observedOn.value()`), group-split key = `locationKey()`; mixed site-scoped/page-scoped inputs
  promote cleanly with the null-page occurrence winning representativeness ties.
- **`version` increments on every re-record (plan-verbatim SQL).** Accepted semantics: "times
  recorded". D26 idempotency holds for rows, counts and `occurrence_count`; a reclaim does not
  corrupt anything, but any future optimistic locking must not read `version` as unchanged.
- **The per-row upsert is deliberately N+1** (plan-verbatim: `batchUpdate` cannot return keys).
  Fine at hundreds of findings; the occurrence insert is the batched path.
- **`rechecked` counts subjects actually re-verified** (post-`normalize`), not the raw suspect
  set. Cache write-back is external-subjects-only (`sameSiteAs` filter). With the default
  `reverify-attempts=2` only one sleep occurs — the doubling engages at attempts ≥ 3.
- **The run log line counts `observedTotal() - NEW`** as "weiterhin offen" (= REGRESSED + KNOWN +
  STILL_OPEN). The first implementation subtracted `FIXED` again (possible negative); review
  caught it.
- **A budget-capped run legitimately creates new fingerprints.** Run 3 (`maxPages=2`) resolves
  in-coverage findings and re-materialises the site-wide `/assets/fehlt.png` finding as two
  page-level ones. The acceptance assertions are scoped per run id so run 3's legitimate changes
  cannot pollute the run-1/run-2 stability checks.
- **The baseline triage sentence is `"Als Ausgangsbestand übernommen."`**, a literal German
  constant on `FindingService` (Plan 5 renders the column verbatim; no message key).
- **Empty-input contract:** `record()` with an empty finding list and full coverage resolves all
  previously active in-coverage findings of the site — pinned by
  `anEmptyRecordWithFullCoverageResolvesEverythingPreviouslyActive`.
- **Tooling:** two subagent sessions hit a stale-target `NoClassDefFoundError` on
  `UrlVerificationServiceTest`; `./mvnw clean test` cleared it both times. Not a code defect, but
  if a fix run fails with that signature, clean first.
- **Browser suite:** `CrawlRunExecutorTest` now crawls the fixture three times in `@BeforeAll`
  (11 tests, ~14 s) — it is the class to extend, not a fourth browser class.
